...
class empsDataTable extends DataTable
{
    ...
    public function dataTable(DataTables $dataTables, $query)
    {
        return datatables($query)
            ->addColumn('actions', 'admin.emps.buttons.actions')
    ...
    {               // $emp = emp::where('user_id', '=', auth()->guard('admin')->id())->get();

        if(auth()->guard('admin')->user()->hasRole('super_admin') || auth()->guard('admin')->user()->hasPermission('create_pers') ||  auth()->guard('admin')->user()->hasPermission('create_attendances') ){

            return emp::query()->with(['job','department','user',])->select("emps.*");
        }
        else{
            return emp::query()->with(['job','department','user',])->select("emps.*")  ->where('user_id', '=', auth()->guard('admin')->id())->get();
        }
       ...
       [
        ...
        'title'=>trans('admin.last_name'),
                ...
                [
                    'name'=>'motivation',
                    'data'=>'motivation',
                    'title'=>trans('admin.motivation'),
                ],
				[
                 'name'=>'major',
                 'data'=>'major',
                 'title'=>trans('admin.major'),
		    ],
				[
                 'name'=>'qulification',
                 'data'=>'qulification',
                 'title'=>trans('admin.qulification'),
            ],
