		...
		<div class="actions">
				<div class="btn-group">
						...
						<ul class="dropdown-menu pull-right">
                            @if(auth()->guard('admin')->user()->hasPermission('update_holidaytypes'))

                            <li>
										<a href="{{ aurl('/holidaytypes/'.$id.'/edit')}}"><i class="fa fa-pencil-square-o"></i> {{trans('admin.edit')}}</a>
								</li>
								<li class="divider"> </li>
                            @endif
                                @if(auth()->guard('admin')->user()->hasPermission('read_holidaytypes'))

                                <li>
										<a href="{{ aurl('/holidaytypes/'.$id)}}"><i class="fa fa-eye"></i> {{trans('admin.show')}}</a>
								</li>
                                @endif
                                @if(auth()->guard('admin')->user()->hasPermission('delete_holidaytypes'))
