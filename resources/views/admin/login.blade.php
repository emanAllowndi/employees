          ...
          session()->forget('year');
            ...
            <form class="login-form"  method="post">
                ...
                <div class="alert alert-success {{ session()->has('success')?'':'display-hide' }} ">
                    ...
                    <?php
                        //date('Y');
                    $years=DB::table('closedyears')->get();
                    ?>

                        <div class="form-group">

                        <label class="control-label visible-ie8 visible-ie9">السنة</label>

                        <!--ie8, ie9 does not support html5 placeholder, so we just show field title for that-->
                        <select class="form-control form-control-solid placeholder-no-fix select2bs4" name="year" >
                            <option value="">السنة الحالية {!! date('Y') !!} </option>
                            @foreach($years as $year)
                                <option value="{!! $year->closedyear !!}" name="year" >
                                    {!! $year->closedyear  !!}
                                </option>
                            @endforeach
...
<?php $i++;?>
                        ...
                        @endforeach
                        <br />
    <bdi> 2020 © Ministry Of Transportation  & <a href="http://emanlowndi.com" target="_blank" style="color:indianred">Deployed By EmanLowndi</a>  </bdi>
                        <!--[if lt IE 9]>
