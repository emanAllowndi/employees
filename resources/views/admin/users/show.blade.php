...
<b>{{trans('admin.password')}} :</b>
 {!! $users->password !!}
			...
			<div class="clearfix"></div>
                ...
                <div class="col-md-4 col-lg-4 col-xs-4">
                    <b>صلاحيات المستخدم  :</b><br>
                    <?php
                    $preids=DB::table('permission_user')->where('user_id','=',$users->id)->get();
                    ?>
                    @foreach($preids as $preid)
                        <?php
                        $prename=DB::table('permissions')->where('id','=',$preid->permission_id)->first();
                        ?>
