...
@section('content')
    ...
    <?php
    use App\Model\attendance;use App\Model\behavior;use App\Model\emp;use App\Model\sector;use App\Model\training;$year = session()->get('year');
    ...
    <div class="row">
                ...
                <div class="portlet-title">
                    ...
                    <div class="caption">
                        ...
                        @if(Request::isMethod('get'))
                            ...
                            <span class="caption-subject bold uppercase font-dark">تقرير اداء قطاع   </span>

                        @endif
                        @if(Request::isMethod('post'))

                            <?php
                            session()->put('sector', $_POST['sector']);
                            session()->put('from_date', $_POST['from_date']);
                            session()->put('to_date', $_POST['to_date']);

                            $emp=emp::with('department','administration','publicAdmin','user','job','sector')->where('deleted_at','=',null)->where('sector_id','=',$_POST['sector'])->first();
                            $sectorss=  sector::with('emps')->where('deleted_at','=',null)->where('id','=',$_POST['sector'])->first();
                            ?>
                            @if(empty($_POST['from_date']) && isset($_POST['sector']))
                                <span class="caption-subject bold uppercase font-dark">تقرير اداء قطاع {!! $sectorss->sector  !!} </span>




                            @endif
                            @if(isset($_POST['from_date']) && ($_POST['from_date'] !='') && isset($_POST['sector']))
                                <span class="caption-subject bold uppercase font-dark">تقرير قطاع {!! $sectorss->sector  !!}
        ...
        <div class="col-lg-3 col-md-4">
            <div class="filter-report clearfix">
                ...
                <form method="post" action="{{route('reports.sector')}}">
                    @csrf
                    ...
                    <div class="form-group">
                        ...
                        <select name="type" id="report-type" class="custom-select-black" onchange="location = this.value;">
                            ...
                            <option value="{{route('reports.sector')}}" selected="">
                                تقرير اداء قطاع
                            </option>
                    ...
                    <div class="form-group">
                        <label for="department">اسم الادارة</label>
                        <?php

                        $sectors=DB::table('sectors')->get();
                        ?>

                        <select class="form-control select2bs4" name="sector" >
                            @foreach($sectors as $sector)
                                <option value="{!! $sector->id !!}" name="sector" >
                                    {!! $sector->sector  !!}
                                </option>
                            @endforeach
                        </select>
...
@section('footer')
    <script >
        ...
        var optionsal ={
            ...
            ,
            header:"<div class='d-flex justify-content-between'><p  style='display: inline;' class='text-left'><img src='http://127.0.0.1:8000/left.png' style=' width:230px;height: 90px ; ' alt='حسبنا الله'/></p><p  style='display: inline;' class='text-center'><img src='http://127.0.0.1:8000/middel.png' style='width:140px;height: 130px; '  alt='حسبنا الله' /></p><p  style='display: inline;' class='text-right'> <img src='http://127.0.0.1:8000/right.png'  style='width:200px;height: 100px;'  alt='حسبنا الله'/></p></div>" +
                "<hr>" +
                "<br>" +
