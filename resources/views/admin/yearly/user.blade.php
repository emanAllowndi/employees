...
@section('content')
    <?php
    use App\Model\attendance;use App\Model\behavior;use App\Model\emp;use App\Model\training;use App\Model\user;$year = session()->get('year');
    ...
    <div class="row">
                ...
                <div class="portlet-title">
                    ...
                    <div class="caption">
                        ...
                        @if(Request::isMethod('get'))
                            ...
                            <span class="caption-subject bold uppercase font-dark">تقرير اداء فريق مسؤول مباشر    </span>

                        @endif
                        @if(Request::isMethod('post'))

                            <?php
                            session()->put('user', $_POST['user']);
                            session()->put('from_date', $_POST['from_date']);
                            session()->put('to_date', $_POST['to_date']);

                            $emp=  emp::with('department','administration','publicAdmin','user','job','sector')->where('deleted_at','=',null)->where('user_id','=',$_POST['user'])->first();
                            $userss=  user::with('emps')->where('deleted_at','=',null)->where('id','=',$_POST['user'])->first();
                            ?>
                            @if(empty($_POST['from_date']) && isset($_POST['user']))
                                <span class="caption-subject bold uppercase font-dark">تقرير اداء فريق المسؤول مباشر :  {!! $userss->first_name !!} {!! $userss->middel_name !!} {!! $userss->last_name !!} </span>




                            @endif
                            @if(isset($_POST['from_date']) && ($_POST['from_date'] !='') && isset($_POST['user']))
                                    <span class="caption-subject bold uppercase font-dark">تقرير اداء فريق المسؤول مباشر :  {!! $userss->first_name !!} {!! $userss->middel_name !!} {!! $userss->last_name !!}
                    ...
                    <table class="table table-bordered table-striped">
                        ...
                        @foreach($allemps as $allemp)
                            ...
                            @if(empty($_POST['from_date']))
                                ...
                                <?php
                                $attendances= attendance::with('emp')->where('deleted_at','=',null)->where('ayear','=',$year)->where('emp_id','=',$allemp->id)->get();
        ...
        <div class="col-lg-3 col-md-4">
            <div class="filter-report clearfix">
                ...
                <form method="post" action="{{route('reports.user')}}">
                    @csrf
                    ...
                    <div class="form-group">
                        ...
                        <select name="type" id="report-type" class="custom-select-black" onchange="location = this.value;">
                            ...
                            <option value="{{route('reports.user')}}"  selected="">
                    ...
                    <div class="form-group">
                        <label for="department">اسم المشرف المسؤول</label>
                        <?php
                        ...
                        <select class="form-control select2bs4" name="user" >
                            @foreach($users as $user)
                                <option value="{!! $user->id !!}" name="user" >
                                    {!! $user->first_name  !!} {!! $user->middel_name  !!} {!! $user->last_name  !!}
                                </option>
                            @endforeach
                        </select>
